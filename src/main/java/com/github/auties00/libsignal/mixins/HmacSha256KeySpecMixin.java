...
@ProtobufMixin
public final class HmacSha256KeySpecMixin {
    @ProtobufDeserializer
    public static SecretKeySpec deserialize(ByteBuffer buffer) {
        if (buffer.hasArray()) {
            return new SecretKeySpec(buffer.array(), buffer.position(), buffer.remaining(), "HmacSHA256");
        } else {
            var array = new byte[buffer.remaining()];
            buffer.get(array);
            return new SecretKeySpec(array, "HmacSHA256");
        }
    }
