...
public final class SignalRootKey {
    ...
    public Chain createChain(HKDF hkdf, SignalIdentityPrivateKey ourRatchetKey, SignalIdentityPublicKey theirRatchetKey) {
        try {
            var sharedSecret = Curve25519.sharedKey(ourRatchetKey.toEncodedPoint(), theirRatchetKey.toEncodedPoint());
            var senderDerivedSecrets = hkdf.deriveSecrets(sharedSecret, key.toEncodedPoint(), CHAIN_INFO, 64);
